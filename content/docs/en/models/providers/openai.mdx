...
GPT-5.4 is a model in the OpenAI GPT-5 family, featuring an approximately 1.05 million token context window and up to 128k output tokens. It supports configurable reasoning effort (`none`, `low`, `medium`, `high`, `xhigh`) and text verbosity (`low`, `medium`, `high`) for general-purpose and agentic workflows. The default reasoning effort is `none` for lower-latency interactions.

### gpt-5.4

*   **Key Features:** Large context window, advanced reasoning options, multimodal input (text, image), web search
*   **Context Window:** ~1,050,000 tokens (based on OpenAI official specification)
...
## GPT-5.3 Series Models

GPT-5.3-Codex combines the coding performance of GPT-5.2-Codex with the reasoning capabilities of the GPT-5.2 series. It is designed for long-horizon agentic coding and is available at the same pricing as GPT-5.2-Codex (input $1.75 / output $14.00 per 1M tokens).

### gpt-5.3-codex

*   **Key Features:** Agentic coding, reasoning, image input, text generation, optional search grounding
...
*   **For the largest context and latest GPT-5 generation**: `gpt-5.4` (Pro)
*   **For best overall performance, coding, agentic tasks, and highest reasoning**: `gpt-5.2` (Pro) - OpenAI's most capable model
*   **For long-horizon agentic coding (latest Codex)**: `gpt-5.3-codex` (Pro)
...
For Pro users, we recommend **gpt-5.4** when an approximately 1.05M token context is needed, and **gpt-5.2** for other general-purpose workloads as the primary model in Giselle. GPT-5.2 is one of OpenAI's most capable models, offering state-of-the-art performance in general intelligence, instruction following, accuracy, multimodality (especially vision), code generation, tool calling, and spreadsheet understanding. GPT-5.2 is designed to be highly reliable and accurate, with 30% fewer errors compared to GPT-5.1 Thinking.

**Configuring GPT-5.4:**
*   **Reasoning Effort:** `none` (default), `low`, `medium`, `high`, `xhigh`. Use `none` for low-latency interactions, and `medium` or higher for deeper reasoning.
*   **Text Verbosity:** `low`, `medium` (default), `high` to control output length.
...
For developers and teams working on complex, long-horizon coding projects, **gpt-5.3-codex** or **gpt-5.2-codex** are well-suited choices. They are specifically optimized for agentic coding tasks in Codex-like environments, with text verbosity fixed to `medium`. GPT-5.3-Codex is available at the same pricing as GPT-5.2-Codex.

By combining these models in workflows, you can leverage their specific strengths. For example, use `gpt-5.4` for tasks requiring a large context, `gpt-5.2` for state-of-the-art reasoning and general capabilities, `gpt-5.3-codex` or `gpt-5.2-codex` for agentic coding tasks, `gpt-5-mini` for cost-efficient tasks, or `gpt-5-nano` for rapid, high-volume operations.
