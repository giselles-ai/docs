...
Claude Opus 4.6 is Anthropic's most intelligent model, purpose-built for building agents and coding. It delivers state-of-the-art performance on agentic coding tasks, deep search, financial analysis, and legal reasoning. The model features enhanced planning capabilities, stronger error detection in code reviews, and more reliable operation in larger codebases. With a 1M token context window and up to 128k output tokens, Opus 4.6 significantly reduces "context rot" performance degradation in long conversations. It supports text, PDF, and image inputs, and its reasoning capability makes it particularly powerful for multi-step thought processes.
...
Claude Sonnet 4.6 is the latest model in the Sonnet series, featuring a 1 million token context window and up to 64k output tokens. It supports text, PDF, and image inputs, with configurable temperature and thinking (extended thinking) settings in Giselle. Well-suited for workflows that handle long documents or conversations, and tasks requiring a balance of coding and reasoning capabilities. Requires a Pro plan.
...
*   **For the most intelligent agents, coding, and complex reasoning**: `claude-opus-4.6`
*   **For advanced coding, agents, and creative tasks**: `claude-opus-4.5`
*   **For large context and balanced high-performance (latest Sonnet)**: `claude-sonnet-4.6`
...
For detailed specifications, performance benchmarks, or additional assistance, please refer to the [Anthropic API Documentation](https://docs.anthropic.com/en/docs/about-claude/models/all-models) and the [Claude Opus 4.6 announcement](https://www.anthropic.com/news/claude-opus-4-6).
