---
title: Giselle SDK
description: Learn how to use the Giselle SDK to integrate your apps with external applications and services.
---
...
| `schema` | `StandardSchemaV1` | No | - | Validation schema for the output. [Standard Schema v1](https://standardschema.dev/) compatible (Zod, Valibot, etc.). Only applied when the End Node has JSON (Structured Output) configured. |

**Returns:**

The `task` type in the return value differs depending on the End Node's Output Format setting.
...
```typescript
{
  task: {
    ...
    status: "completed" | "failed" | "cancelled";
    outputType: "object";
    output: T; // Typed by schema if provided, otherwise Record<string, unknown>
    steps: Array<{
      title: string;
      status: string;
      items: Array<{
...
4. You'll see a complete code snippet with your `appId` already filled in. Simply copy this code and use it in your application.
5. If the End Node has JSON (Structured Output) enabled, the Code tab also displays a code snippet with schema definitions and a preview of the Response JSON structure
...
| `SchemaValidationError` | Schema validation failed for the output |
...
When the End Node has JSON (Structured Output) enabled, `task.outputType === "object"` and `task.output` contains the structured result.  
Pass a [Standard Schema v1](https://standardschema.dev/) compatible schema to the `schema` parameter to automatically validate the output and get type inference.

Supported libraries: Zod (3.24+), Valibot (1.0+), ArkType (2.0+), Yup (1.7+), Effect Schema (3.13+)
...
async function runMyAppWithStructuredOutput(userInput: string) {
  const { task } = await client.apps.runAndWait({
    appId: "app_xxxxx",
    input: { text: userInput },
    schema: BlogCardSchema,
  });

  if (task.status === "completed" && task.outputType === "object") {
    // task.output is typed based on BlogCardSchema
    return task.output;
  }
