...
1.  **Connection Name**: Give your connection a short, descriptive name for easy identification (e.g., "giselle-db-preview", "Prod-DB"). You'll use this to link other nodes.
...
Task:
Please find out the total number of registered users from the database.

Instructions:
1. First, use the `getTableStructure` tool to inspect the database and identify the table that contains user information.
2. Once you have identified the correct table, write and execute a SQL query using the `query` tool to count the total number of rows in that table.
3. Finally, report the total count of users in your response.
```
...
5.  Return a final answer based on the query result, such as: "The total number of registered users is 1,452."
```