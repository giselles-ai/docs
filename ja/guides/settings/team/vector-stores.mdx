...
- **GitHub Repositories**: GitHub リポジトリのコード、Issues、Pull Request を取り込み

## Document Vector Stores

<Note>
ページ左側のサイドバーに "Document" と "GitHub" のナビゲーションが表示されます。
</Note>
...
Document Vector Stores の作成可能数は、チームのサブスクリプションプランによって制限されています。

#### 各プランの制限

| プラン | 作成可能な Document Vector Stores 数 |
|--------|-------------------------------------|
| Free   | 0個（利用不可）                      |
| Pro    | 5個                                 |
| Team   | 20個                                |
...
Document Vector Stores ページでは、現在の使用状況が表示されます：

- **使用数 / 最大数**: 例："3 / 5 stores"
- **残り数**: 例："2 Document Vector Stores remaining in your Pro plan."
...
1. **アラートが表示されます**:
   ...
   - メッセージ："Delete an existing store or upgrade your plan in Team Settings to add more Document Vector Stores."

2. **New Vector Store ボタンが無効化されます**:
   - ボタンにカーソルを合わせると、ツールチップで理由が表示されます
   - "You've reached the number of Document Vector Stores included in your plan."

3. **対処方法**:
   - 既存の Document Vector Store を削除する
   - [チーム設定](/ja/guides/settings/team/general)でプランをアップグレードする

#### Free プランの場合

Free プランでは Document Vector Stores が利用できません：

1. **ロック状態が表示されます**:
   - タイトル："Document Vector Stores are locked"
   - メッセージ："Upgrade to Pro or Team to ingest knowledge bases and connect Document Vector Stores to your agents."
...
GitHub Vector Stores の登録可能数は、チームのサブスクリプションプランによって制限されています。

#### 各プランの制限

| プラン | 登録可能な GitHub Repositories 数 |
|--------|----------------------------------|
| Free   | 0個（利用不可）                   |
| Pro    | 3個                              |
| Team   | 10個                             |
...
GitHub Repositories ページでは、現在の使用状況が表示されます：

- **使用数 / 最大数**: 例："2 / 3 repositories"
- **残り数**: 例："1 GitHub Repository remaining in your Pro plan."
- **プラン名**: 現在のプラン（Free、Pro、Team）
...
1. **アラートが表示されます**:
   - タイトル："Maximum capacity reached"
   - メッセージ："Delete an existing repository or upgrade your plan in Team Settings to add more GitHub Vector Stores."

2. **Register Repository ボタンが無効化されます**:
   - ボタンにカーソルを合わせると、ツールチップで理由が表示されます
...
Free プランでは GitHub Vector Stores が利用できません：

1. **ロック状態が表示されます**:
   - タイトル："GitHub Vector Stores are locked"
   - メッセージ："Upgrade to Pro or Team to index GitHub repositories and connect them to your agents."
...
- **Issues**: Issue の本文、コメント、ディスカッションを取り込み（オプション）
...
- **Document Vector Store Node**: アップロードしたドキュメントを検索・利用
- **GitHub Vector Store Node**: GitHub リポジトリから取り込んだコード、Issues、Pull Request を検索・利用
