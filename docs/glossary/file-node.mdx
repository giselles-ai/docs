...
*   **PDF and Text files** can be connected to text-based or multimodal models (like Claude or GPT-4o) for tasks like analysis, summarization, or Q&A.
*   **Image files** can be connected to multimodal models that accept image inputs (like `gpt-4o` or `gemini-2.5-pro`) for analysis, or to image generation models for transformation tasks.
