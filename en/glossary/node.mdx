...
Trigger Node is the starting point for running the workflow built in the Giselle App. It initiates the execution of the connected nodes in a sequence.
...
### GitHub Vector Store Node & Vector Query Node

Giselle supports advanced data retrieval and search capabilities through the **GitHub Vector Store Node** and the **Vector Query Node**.
...
- **Vector Query Node**: The Vector Query Node is designed to execute search queries against connected data sources, such as the GitHub Vector Store Node. It retrieves the most relevant information (text chunks) from your vectorized repositories, which can then be used as dynamic context for AI generation or further processing. For more information, see the [Vector Query Node documentation](vector-query-node).

**Typical Usage:**
1. Add a GitHub Vector Store Node and configure it with your repository.
2. Connect its output to a Vector Query Node.
3. Use the Vector Query Node to search your repository's content and pass the results to other nodes, such as a Generator Node, for context-aware AI workflows.
