...
description: "Learn how Start and End Nodes in Giselle define the boundaries of your workflow, establishing where user input enters and where the final output is collected."
---
...
Each parameter becomes an output port on the Start Node, allowing you to connect the user's input to other nodes in your workflow.
...
The Start Node's **outputs** correspond to the parameters you've defined. Each parameter creates one output port containing the value provided by the user when running the app. These outputs can be connected to any compatible node's input.
...
*   **Output Definition**: Specify which node outputs to include in the app's final result. You can connect multiple upstream nodes.
*   **Single Instance**: Each workflow can have only one End Node. It cannot be duplicated.
*   **Dynamic Inputs**: Unlike other nodes, the End Node's inputs are created dynamically when you connect upstream nodes.
...
*   Nodes without outputs (like the Start Node) cannot be connected
*   When you connect a node, all of its outputs are connected at once

### Input of the End Node

The End Node's **inputs** are created dynamically based on the connections you make. Each connected upstream node's outputs become inputs on the End Node, which are then presented as the final output of your app.

## Visual Appearance

Both Start and End Nodes share a distinctive **capsule-shaped design** that sets them apart from other nodes:

*   **Start Node**: Has output handles on the right side for connecting to downstream nodes
*   **End Node**: Has input handles on the left side for receiving data from upstream nodes
