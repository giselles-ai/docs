...
You can also set the **Output Format** to **JSON** in the Generator Node's **Advanced options** and define a Structured Output schema via the UI to reliably control the actual output format.  
See the [Generator Node documentation](/en/glossary/generator-node#advanced-options) for more details.
